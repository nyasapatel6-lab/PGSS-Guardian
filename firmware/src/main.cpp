...
const char* GMAIL_PASS    = "dpeoockhqpgnlrqb";
const char* GMAIL_TO      = "ashish.works@gmail.com";
const char* GDRIVE_UPLOAD_URL = "";
const char* GDRIVE_OTA_URL    = "";

const char* SUPABASE_URL  = "https://qgrbsnnuwxgvdoaqicze.supabase.co";
const char* SUPABASE_ANON = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFncmJzbm51d3hndmRvYXFpY3plIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODE3NjM5MzIsImV4cCI6MjA5NzMzOTkzMn0.0lNuif1wo8MAzJn9zoPD15t_IYTiMaxCcm016OcJlm4"; // same anon key already used in constants/theme.ts
const char* PGSS_USER_NAME = "Kajal";
const char* PGSS_GUARDIAN  = "";   // guardian contact string, optional
...
bool sbPostAlert(String type, int severity, bool haveGps, double lat, double lng) {
  ...
  String gpsStr = haveGps ? (String(lat, 6) + "," + String(lng, 6)) : "";
  String payload = "{";
  payload += "\"type\":\"" + type + "\",";
  payload += "\"severity\":" + String(severity) + ",";
  payload += "\"gps\":" + (haveGps ? ("\"" + gpsStr + "\"") : "null") + ",";
  payload += "\"user_name\":\"" + String(PGSS_USER_NAME) + "\",";
  payload += "\"guardian\":\"" + String(PGSS_GUARDIAN) + "\",";
  ...
  Serial.println(ok ? "Supabase: alert row inserted"
                     : ("Supabase: alert insert FAILED (" + String(code) + ") " + http.getString()));
  http.end();
  return ok;
}

bool sbPostVitals(int battery, float fallScore, bool haveGps, double lat, double lng) {
  if (WiFi.status() != WL_CONNECTED) return false;

  WiFiClientSecure client;
  client.setInsecure();
  HTTPClient http;
  String url = String(SUPABASE_URL) + "/rest/v1/vitals?on_conflict=id";
  http.begin(client, url);
  http.addHeader("Content-Type", "application/json");
  http.addHeader("apikey", SUPABASE_ANON);
  http.addHeader("Authorization", "Bearer " + String(SUPABASE_ANON));
  ...
  payload += "\"fall_score\":" + String(fallScore, 3) + ",";
  payload += "\"gps_lat\":" + (haveGps ? String(lat, 6) : "null") + ",";
  payload += "\"gps_lng\":" + (haveGps ? String(lng, 6) : "null") + ",";
  payload += "\"device_ip\":\"" + WiFi.localIP().toString() + "\"";
  payload += "}";

  int code = http.POST(payload);
  bool ok = (code == 200 || code == 201 || code == 204);
  Serial.println(ok ? "Supabase: vitals updated"
                     : ("Supabase: vitals update FAILED (" + String(code) + ") " + http.getString()));
  http.end();
  return ok;
}
...
void sendAlerts() {
  ...
  sbPostAlert(
    (alert_type == "MANUAL") ? "manual" : "fall",
    severity_score,
    gps_valid, gps_lat, gps_lng
  );
  ...
  for (int i = 0; i < WINDOW_SIZE; i++) {
    ...
    for (int j = 0; j < N_FEATURES; j++) {
      tf_input->data.f[i * N_FEATURES + j] =
        (window_buffer[buf_i][j] - SCALER_MEAN[j]) / SCALER_STD[j];
    }
  }
  esp_task_wdt_reset();
...
void setup() {
  ...
  // --- New: battery monitor + OLED. Deliberately last in setup() so a
  // failure here (e.g. OLED not wired yet) never blocks fall detection,
  // GPS, WiFi, or the TFLite model from initializing above this point.
  initBatteryMonitor();
  initOLED();
}
...
void loop() {
  ...
  // --- New: push a live vitals row to Supabase every ~20s, non-blocking.
  static unsigned long last_vitals_push = 0;
  if (now - last_vitals_push > 20000) {
    last_vitals_push = now;
    sbPostVitals(getBatteryPercent(), last_fall_score, gps_valid, gps_lat, gps_lng);
  }

  // --- New: battery % + OLED refresh, every 2s, non-blocking (same
  // millis() pattern as the rest of this loop -- no delay() added).
  static unsigned long last_oled_update  = 0;
  static bool           low_batt_alerted = false;
  if (now - last_oled_update >= 2000) {
    last_oled_update = now;
    int batt_pct = getBatteryPercent();
    updateOLED(batt_pct, WiFi.status() == WL_CONNECTED, fall_active, sleep_mode);

    // One-shot warning blink at 15% -- resets once battery is back above 20%
    // (charged), so it can re-fire on the next discharge cycle instead of
    // going silent forever after the first trip.
    if (batt_pct <= 15 && !low_batt_alerted) {
      low_batt_alerted = true;
      Serial.println("WARN: Battery low (<=15%)");
      blinkLED(4); // distinct from sleep(2)/awake(3)/panic patterns already in use
    } else if (batt_pct > 20) {
      low_batt_alerted = false;
    }
  }
