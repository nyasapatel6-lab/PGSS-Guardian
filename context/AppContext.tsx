...
import { sbLogAlert, sbUpdateVitals, sbListenCommands, sbListenAlerts } from '../lib/supabase';
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [lastVitalsUpdate, setLastVitalsUpdate] = useState(Date.now());
  ...
  useEffect(() => {
    ...
    const unsubCmds = sbListenCommands(toggleSleep, triggerPanic);
    const unsubAlerts = sbListenAlerts((newAlert) => {
      // Don't alert if we just sent it (simulation or matching IP)
      if (newAlert.device_ip === deviceIPRef.current) return;
      Alert.alert('⚠️ Remote Alert Received', `Alert Type: ${newAlert.type}\nSeverity: ${newAlert.severity}`);
      setLogData((prev) => [{ id: Date.now(), type: 'critical', title: `Remote ${newAlert.type} alert`, sev: newAlert.severity, gps: newAlert.gps || '—', ts: Date.now() }, ...prev]);
    });
  ...
  const logout = useCallback(async () => {
    ...
    await AsyncStorage.removeItem('pgss_u');
  }, []);
